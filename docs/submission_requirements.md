...
## 公式サンプル PDF との実測比較 (2026-08-06)

`pdftotext -bbox-layout` で両者のテキスト座標を突き合わせた結果。

### タイトルブロックの高さ — **合わせ済み**

| | タイトル開始 y | 本文開始 y | ブロック高 |
| --- | ---: | ---: | ---: |
| 公式サンプル (タイトル 1 行) | 70.1 | 190.5 | **120.4 pt** |
| 修正前の本 repo (タイトル 2 行) | 72.4 | 256.5 | **184.1 pt** |
| **修正後の本 repo** (タイトル 2 行) | 72.4 | 172.2 | **99.9 pt** |

- 原因は `center` 環境の `\topsep` (上下 10 pt) に文書全体の `\parskip` (12.6 pt)
  が重なり、ブロック間が **32.5 pt** になっていたこと (サンプルは約 15 pt)。
- 対処: `center` 環境をやめて素の `\centering` で組み、`\parskip` を明示的に
  打ち消してからテンプレ相当の間隔だけ足す (`tasti2026.sty` の title-block 節)。
  **約 84 pt = 本文 6.7 行分**を回収した。
- テンプレ規定との対応も維持: affiliation は著者行の直後 (空行なし)、
  email はその次の行、keywords と本文の間は 1 行。
...
| --- | ---: |
| 公式サンプル | 約 **15 pt** |
| 本 repo | **12.6 pt** (10.5 pt × 1.2) |

要件の記載は "Line spacing: Single" のみ。Word の single spacing は Times New
Roman 10.5 pt で約 12.1 pt なので、**12.6 pt は "Single" の忠実な解釈**であり、
かつ内容が入る。厳密な見た目の一致を求められたら `tasti2026.sty` の
`\normalsize` 再定義の第 2 引数を 15 にすること (本文が約 19% 伸びる)。
